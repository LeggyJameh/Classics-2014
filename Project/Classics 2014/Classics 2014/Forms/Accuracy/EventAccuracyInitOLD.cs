...
{
    // Fixed 28/10/14.
     partial class EventAccuracyInitOLD : UserControl // Options menu when making a Accuracy Event. Allows user to set rules, setup competitors, etc.
    {
        ...
        string EventName = "";
        #region Structs
        Ruleset.AccuracyRules Fai = new Ruleset.AccuracyRules
        {
            maxScored = 16,
            noOfCompetitorsPerTeam = 5,
            preset = "FAI",
        };
        Ruleset.AccuracyRules CISM = new Ruleset.AccuracyRules
        {
            maxScored = 20,
            noOfCompetitorsPerTeam = 5,
            preset = "C.I.S.M",
        };
        Ruleset.AccuracyRules CISMJunior = new Ruleset.AccuracyRules
        {
            maxScored = 20,
            noOfCompetitorsPerTeam = 1,
            preset = "C.I.S.M Junior",
        };
        Ruleset.AccuracyRules NationalSeniors = new Ruleset.AccuracyRules
        {
            maxScored = 20,
            preset = "National Seniors",
        };
        Ruleset.AccuracyRules NationalPOPS = new Ruleset.AccuracyRules
        {
            maxScored = 100,
            preset = "National POPS",
        };
        Ruleset.AccuracyRules NationalIntermediates = new Ruleset.AccuracyRules
        {
            maxScored = 500,
            noOfCompetitorsPerTeam = 1,
            preset = "National Intermediates",
        };
        Ruleset.AccuracyRules NationalJuniors = new Ruleset.AccuracyRules
        {
            maxScored = 2500,
            noOfCompetitorsPerTeam = 1,
            preset = "National Juniors",
        };
        Ruleset.AccuracyRules Paragliding = new Ruleset.AccuracyRules
        {
            maxScored = 2500,
            preset = "Paragliding",
        };
        #endregion
        public EventAccuracyInitOLD(TabControl Main, Accuracy_Event aEvent)
        {
            InitializeComponent();
            tabControl = Main;
        ...
        public EventAccuracyInitOLD(TabControl Main, Accuracy_Event aEvent, Ruleset.AccuracyRules LoadRules, string LoadEventName, DateTime LoadDate, List<Competitor> LoadSelectedCompetitors)
        {
            InitializeComponent();
            tabControl = Main;
            Connected_Event = aEvent;
        ...
        private void LoadEvent(DateTime LoadedDate, Ruleset.AccuracyRules LoadedRules)
        {
        ...
        private void PopulateExistingCompetitors()
        {
            ...
            for (int i = 0; i < SelectedTeams.Count; i++)
			{
                ExistingCompetitors = Connected_Event.SQL_Controller.GetCompetitorsByTeam(SelectedTeams[i], SelectedCompetitors);
			}
            UpdateCompetitorGridsFromData();
        }
        ...
        private void PopulateExistingTeams()
        {
            ExistingTeams.Clear();
            listBoxExistingTeams.Items.Clear();
            ExistingTeams = Connected_Event.SQL_Controller.GetTeams(false);
            for (int i = 0; i < ExistingTeams.Count; i++)
            {
                listBoxExistingTeams.Items.Add(ExistingTeams[i]);
            }
                ...
                if (textBoxCompetitorNationality.Text != "" && textBoxCompetitorNationality.Text.Length < 255)
                {
                    if (comboBoxCompetitorTeam.SelectedItem != null)
                    {
                        ...
                        if (CheckCompetitorIsValid(CurrentCompetitor.name))
                        {
                            ...
                            {
                                CurrentCompetitor.ID = Connected_Event.SQL_Controller.CreateCompetitor(CurrentCompetitor);
